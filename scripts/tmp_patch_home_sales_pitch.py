...
old_getting = '''const gettingStarted = [
...
new_getting = '''const gettingStarted = [
...
if old_getting not in s:
    raise SystemExit("gettingStarted block not found")
s = s.replace(old_getting, new_getting, 1)

start = s.index("function HeroValueStrip() {")
end = s.index("\nfunction InfoButton", start)
new_strip = '''function HeroValueStrip() {
  return (
...
replacements = {
    '<p className="ua3-category">DAILY REFERENCE DATA</p>': '<p className="ua3-category">DAILY NETWORK-STATE CLASSIFICATION</p>',
    '<h1 id="hero-title" className="ua3-display">Urd Atlas is a daily network-state report for the Bitcoin, Ethereum, Arbitrum and Base networks.</h1>': '<h1 id="hero-title" className="ua3-display">One daily network regime for Bitcoin, Ethereum, Arbitrum and Base — with the evidence underneath.</h1>',
    '<p className="ua3-body ua3-hero-copy">Built for analysts and data teams that need to explain why a number changed, not predict what it becomes.</p>': '<p className="ua3-body ua3-hero-copy">Urd Atlas classifies how the network behaved relative to its own recent history: STABLE, HEATING, CONGESTED or CHEAP. It is a network regime, not a market regime.</p>',
    '<a href="#today-status" className="ua3-button ua3-button-primary">See today&apos;s status →</a>': '<a href="#today-status" className="ua3-button ua3-button-primary">See today&apos;s network state →</a>',
    '<section className="ua3-section ua3-start" aria-labelledby="start-title"><div className="ua3-wrap"><h2 id="start-title" className="ua3-step-title">Get started.</h2>': '<section className="ua3-section ua3-start" aria-labelledby="start-title"><div className="ua3-wrap"><h2 id="start-title" className="ua3-step-title">What you are buying.</h2>',
    '<p className="ua3-label ua3-step-label">Step 3</p>': '<p className="ua3-label ua3-step-label">Published network state</p>',
    '<p className="ua3-label ua3-step-label">Step 4</p>': '<p className="ua3-label ua3-step-label">Delivered evidence</p>',
    '<p className="ua3-label ua3-step-label">Step 5</p>': '<p className="ua3-label ua3-step-label">Plans</p>',
}
for old, new in replacements.items():
    if old not in s:
        raise SystemExit(f"replacement target not found: {old[:80]}")
    s = s.replace(old, new, 1)

pricing_marker = '      <section id="pricing" className="ua3-section ua3-pricing" aria-labelledby="pricing-title">'
if pricing_marker not in s:
    raise SystemExit("pricing marker not found")
build_buy = '''      <section className="ua3-section ua3-start" aria-labelledby="build-buy-title">
...
'''
s = s.replace(pricing_marker, build_buy + pricing_marker, 1)

boundary = '<div className="ua3-compliance-row" aria-label="Product boundary"><span className="ua3-compliance-pill">No price data</span><span className="ua3-compliance-pill">No forecasts</span><span className="ua3-compliance-pill">No recommendations</span></div>'
if boundary not in s:
    raise SystemExit("boundary row not found")
s = s.replace(boundary, boundary + '\n            <p className="ua3-body-small">Transparent rules, chain-aware methodology and empirical robustness checks. <Link href="/validation">See the validation evidence →</Link></p>', 1)
