...
import { getMetricDescription, getMetricOptionsForChain } from "@/lib/registry/metricRegistry";
...
// Metric list is now sourced from the registry (v1.1) to avoid component hard-coding.
...
export function ChainDetail({ chain }: { chain: string }) {
  ...
  const metricOptions = useMemo(() => {
    // Keep a stable set even during brief "unknown chain" states.
    return getMetricOptionsForChain(chainId ?? "bitcoin");
  }, [chainId]);

  const [metricKey, setMetricKey] = useState<string>(() => metricOptions[0]?.key ?? "tx_count_daily__ma7");

  // Ensure selected metric is valid for the chain (or when registry changes).
  useEffect(() => {
    if (!chainId) return;
    if (metricOptions.length === 0) return;
    const ok = metricOptions.some((m) => m.key === metricKey);
    if (!ok) setMetricKey(metricOptions[0].key);
  }, [chainId, metricOptions, metricKey]);
          ...
          <div className="mt-4">
            <InfoBox
              ...
              basic={
                ...
                "A smoothed time series (moving average) for the selected on-chain metric."
              }
              advanced={
                (getMetricDescription(metricKey, "advanced") ??
                  "Each point is read from /derived/<chain>/<date>.json. Moving averages are precomputed in the pipeline (e.g., MA7 / MA30).") +
                " Chart window currently defaults to 180 days ending at the chain’s derived as-of date."
              }
            />
          </div>
        </div>
