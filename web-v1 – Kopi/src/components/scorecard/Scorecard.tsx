...
export type ScorecardDimension = {
  ...
  score: number | null; // 0..100, 50 neutral (may be null if inputs missing)
...
export function ScorecardView({
  ...
  return (
    <section className="rounded-2xl border border-ui-border bg-ui-surface p-5 shadow-sm">
      ...
      {/* web6: purpose/value box for panels */}
      <PanelPurpose
        className="mt-4"
        whatThisShows={
          "A descriptive, three-axis operating profile built from on-chain inputs for Demand, Friction, and Capacity. " +
          "Each axis is normalized to the chain’s own historical behavior over the stated window; low coverage reduces confidence and pulls scores toward neutral (50)."
        }
        commonlyUsedFor={[
          "Summarizing the chain’s operating state without price data (demand vs friction vs capacity).",
          "Comparing snapshots over time using a consistent 0–100 scale anchored to the chain’s own history.",
          "Identifying which axis is driving changes before drilling into underlying drivers/metrics.",
          "Communicating data quality constraints via coverage/confidence mechanics (neutral pull when inputs are missing).",
        ]}
      />
