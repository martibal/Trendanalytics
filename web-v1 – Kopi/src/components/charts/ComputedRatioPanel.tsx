...
export function ComputedRatioPanel(props: {
  ...
  useEffect(() => {
    ...
    async function run() {
      if (!numeratorKnown || !denominatorKnown) {
        setLoading(false);
        setErr(
          `Unknown/undocumented metric key(s): ${!numeratorKnown ? props.numeratorMetric : ""} ${
            !denominatorKnown ? props.denominatorMetric : ""
          }`.trim()
        );
        setNumSeries(null);
        setDenSeries(null);
  ...
  return (
    <section className="rounded-3xl border border-ui-border bg-ui-bg/20 p-6 ui-lift">
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Chip
              label="Coverage"
              value={`${computed.coverage.present_days}/${computed.coverage.expected_days} (${Math.round(
                computed.coverage.nonNull_ratio * 100
              )}%)`}
      ...
      {/* web6: purpose/value box for computed panels */}
      <PanelPurpose
        className="mt-4"
        learnMoreHref={hrefs.methodologyHref}
        whatThisShows={
          "A computed ratio time series built from two documented metrics (numerator / denominator). " +
          "It is displayed as Daily, MA7, and MA30 for noise-vs-structure context, plus percentile and z-score for distribution context in the selected window."
        }
        commonlyUsedFor={[
          "Normalizing activity by a denominator to control for scale effects (e.g., per-user or per-transaction ratios).",
          "Detecting persistent shifts in the ratio (MA7 vs MA30) without relying on price.",
          "Comparing how unusual the current ratio is inside the window (percentile and z-score).",
          "Auditing the underlying components and coverage before interpreting changes (coverage + lag chips).",
        ]}
      />
