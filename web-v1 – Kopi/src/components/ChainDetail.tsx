...
import { useDatasetIndex as usePublishedIndex } from "@/hooks/useDatasetIndex";
...
type Genre = "meta" | "derived" | "gold";

function readCoverageAsof(dataset: any, chain: ChainId, genre: Genre): string | undefined {
  // Web3 source of truth: dataset.coverage[chain][genre].asof
  const v = dataset?.coverage?.[chain]?.[genre]?.asof;
  if (typeof v === "string" && v.length) return v;

  // Legacy fallback (pre-web3)
  const legacy = dataset?.asof_by_genre_chain?.[genre]?.[chain];
...
type GateState =
  ...
  | {
      ...
      reason: "missing_data" | "confidence_below_threshold";
      confidence: number | null;
      threshold: number;
    };

function readConfidence(meta: any): number | null {
  const v = meta?.confidence?.confidence_score;
  return typeof v === "number" && Number.isFinite(v) ? v : null;
}

function computeGate(meta: any, threshold: number): GateState {
  if (!meta) return { status: "unknown_degraded", reason: "missing_data", confidence: null, threshold };
  const c = readConfidence(meta);
  if (c === null) return { status: "unknown_degraded", reason: "missing_data", confidence: null, threshold };
...
/**
 * Web3 copy: keep regime label separate from gate reason.
 * Regime label is overwritten to UNKNOWN/DEGRADED when gated.
 * Gate label should only describe the reason (or OK).
 */
function gateLabel(g: GateState): string {
  if (g.status === "ok") return "OK";
  if (g.reason === "missing_data") return "Missing META inputs";
  return `Confidence < ${(g.threshold * 100).toFixed(0)}%`;
}
...
export function ChainDetail({ chain }: { chain: string }) {
  ...
  const { dataset: ds, gatingThreshold } = usePublishedIndex();
  const explainMode = useUiStore((s) => s.explainMode) as ExplainMode;

  const metaAsof = chainId ? readCoverageAsof(ds as any, chainId, "meta") : undefined;
  const derivedAsof = chainId ? readCoverageAsof(ds as any, chainId, "derived") : undefined;
  const goldAsof = chainId ? readCoverageAsof(ds as any, chainId, "gold") : undefined;
  ...
  const metaForDisplay = useMemo(() => {
    if (!meta) return meta;
    if (gate.status === "ok") return meta;
    const regime = (meta as any)?.regime;
    // Overwrite only the headline label deterministically when gated (web3; no backfill)
    return {
      ...(meta as any),
      regime: { ...(regime ?? {}), label: "UNKNOWN/DEGRADED" },
    };
  }, [meta, gate.status]);
  ...
  const notables = useMemo(() => {
    return buildNotables({ meta: metaForDisplay, explainMode, metricLabelByKey });
  }, [metaForDisplay, explainMode, metricLabelByKey]);
  ...
  const whatToWatch = useMemo(() => {
    return buildWhatToWatch({ meta: metaForDisplay, primaryPanels });
  }, [metaForDisplay, primaryPanels]);
  ...
  const summary = buildInterpretationSummary(metaForDisplay, explainMode);
        ...
        <div className="mt-2 grid grid-cols-1 gap-3 sm:grid-cols-4">
          ...
          <div className="rounded-xl border border-ui-border bg-ui-bg/30 px-3 py-2">
            ...
            <div className="mt-1 text-sm text-ui-text">
              {metaForDisplay?.confidence?.confidence_score != null ? metaForDisplay.confidence.confidence_score.toFixed(2) : "—"}
            </div>
          </div>
          <div className="rounded-xl border border-ui-border bg-ui-bg/30 px-3 py-2">
          ...
          <div className="rounded-xl border border-ui-border bg-ui-bg/30 px-3 py-2">
            <div className="text-[11px] text-ui-faint">Gated</div>
            <div className="mt-1 text-sm text-ui-text">{gateLabel(gate)}</div>
            <div className="mt-1 text-[11px] text-ui-faint">threshold: {Math.round(effectiveGatingThreshold * 100)}%</div>
          </div>
        </div>
      </div>
    </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <div className="mt-1 text-xs text-ui-muted">
            ...
            <span className="text-ui-text">{goldAsof ?? "—"}</span> · Gated:{" "}
            <span className="text-ui-text">{gateLabel(gate)}</span>
          </div>
        </div>
        {metaForDisplay?.regime?.label ? <RegimeBadge label={metaForDisplay.regime.label} /> : null}
      </div>
      ...
      <div className="rounded-2xl border border-ui-border bg-ui-surface p-4">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex flex-wrap gap-2">
            <div className="ui-badge px-3 py-2 text-xs text-ui-muted">
              Regime: <span className="text-ui-text">{metaForDisplay?.regime?.label ?? "—"}</span>
            </div>
            <div className="ui-badge px-3 py-2 text-xs text-ui-muted">
              Demand: <span className="text-ui-text">{metaForDisplay?.regime?.axes?.demand?.trend ?? "—"}</span>
            </div>
            <div className="ui-badge px-3 py-2 text-xs text-ui-muted">
              Friction: <span className="text-ui-text">{metaForDisplay?.regime?.axes?.friction?.trend ?? "—"}</span>
            </div>
            <div className="ui-badge px-3 py-2 text-xs text-ui-muted">
              Capacity: <span className="text-ui-text">{metaForDisplay?.regime?.axes?.capacity?.trend ?? "—"}</span>
            </div>
            <div className="ui-badge px-3 py-2 text-xs text-ui-muted">
              Confidence (7d):{" "}
            ...
            <div className="ui-badge px-3 py-2 text-xs text-ui-muted">
              Gated: <span className="text-ui-text">{gateLabel(gate)}</span>
            </div>
          </div>
        </div>
        ...
        {gate.status !== "ok" ? (
          <div className="mt-3 text-xs text-ui-faint leading-relaxed">
            Gate note: headline regime is shown as{" "}
            <span className="font-semibold text-ui-muted">UNKNOWN/DEGRADED</span> because{" "}
            <span className="font-semibold text-ui-muted">{gateLabel(gate)}</span>. This is deterministic from META +
            contract threshold; no backfill is applied.
          </div>
        ) : null}
