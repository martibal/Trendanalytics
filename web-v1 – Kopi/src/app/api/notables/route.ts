...
function pickZPercentile(meta: unknown, metric: string): { z: number | null; percentile: number | null } {
  ...
  if (!m) return { z: null, percentile: null };

  const metrics = asRecord(m["metrics"]);
  const metricObj = metrics ? asRecord(metrics[metric]) : null;
  const metricObjAlt = asRecord(m[metric]);
  ...
  const z = (zCandidates.find((v) => isNumber(v)) as number | undefined) ?? null;
...
async function loadWindowSeries(
  root: string,
  chain: Chain,
  metric: string,
  start: string,
  ...
  for (const d of present_days) {
    ...
    const daily: number | null = goldDaily && isNumber(goldDaily[metric]) ? (goldDaily[metric] as number) : null;
    ...
    const ma7: number | null =
      derivedMetrics && isNumber(derivedMetrics[ma7Key]) ? (derivedMetrics[ma7Key] as number) : null;
...
function persistenceHint(rows: Row[]) {
  ...
  const n = rows.length;
  if (n < 14) return { ok: false, delta: null as number | null, label: "insufficient window length" as const };
  ...
  if (m1 === null || m2 === null) return { ok: false, delta: null as number | null, label: "insufficient non-null values" as const };
...
export async function GET(req: NextRequest) {
  try {
    ...
    const asof: string | null =
      goldManifest && typeof goldManifest["asof"] === "string" ? (goldManifest["asof"] as string) : null;
    ...
    const availableDaysList: string[] = Array.isArray(availableDaysRaw) ? (availableDaysRaw as unknown[]).filter((d): d is string => typeof d === "string") : [];
    const availableDays = Array.from(new Set(availableDaysList.filter((d) => isValidISODate(d)))).sort();
    ...
    const msg = e instanceof Error ? e.message : "Notables route failed";
    return jsonError("NOTABLES_FAILED", msg, 500);
  }
}