...
export default function ChainPage() {
  ...
  // Charts accordion state (avoid rendering charts in closed <details> to prevent Recharts zero-size warnings)
  ...
  return (
    <div className="mx-auto max-w-6xl p-6">
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <div className="mt-1 text-xs text-white/70">
              This page is descriptive. Charts show observed levels and smoothed context (MA7/MA30). Regimes and notables are
              interpretive summaries and are shown only when freshness, confidence and coverage meet thresholds.
            </div>
          </div>

          <details className="rounded-xl border border-white/10 bg-black/20 px-3 py-2">
            <summary className="cursor-pointer select-none text-xs text-white/70">
              Advanced: quality thresholds & policy
            </summary>
            <div className="mt-2 text-xs text-white/70 space-y-1">
              <div>
                Freshness gate: lag_days &gt; publish_lag_days_policy + 2
              </div>
              <div>
                Confidence gate: confidence_score &lt; 0.5 (if present)
              </div>
              <div>
                Coverage gate: coverage_days &lt; 120 (gold manifest or window length)
              </div>
            </div>
          </details>
        </div>

        {/* Data quality / gating summary (uses existing computed vars) */}
        <div className="mt-4 rounded-xl border border-white/10 bg-black/20 p-4">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <div className="text-sm font-semibold">Data quality</div>
            {typeof isGated !== "undefined" && isGated ? (
          ...
          <div className="mt-2 grid grid-cols-1 gap-2 md:grid-cols-3 text-xs text-white/70">
            <div className="rounded-lg border border-white/10 bg-black/20 p-3">
              <div className="uppercase tracking-wider text-white/50">Freshness</div>
              <div className="mt-1 font-mono text-white/85">
                lag {typeof lagDays !== "undefined" && lagDays !== null ? `${lagDays}d` : "—"} / policy{" "}
                {typeof publishLagPolicy !== "undefined" ? `${publishLagPolicy}d` : "—"}
              </div>
            </div>

            <div className="rounded-lg border border-white/10 bg-black/20 p-3">
              <div className="uppercase tracking-wider text-white/50">Confidence</div>
              <div className="mt-1 font-mono text-white/85">
                {typeof confidenceScore !== "undefined" && confidenceScore !== null ? confidenceScore.toExponential(2) : "—"}
              </div>
            </div>

            <div className="rounded-lg border border-white/10 bg-black/20 p-3">
              <div className="uppercase tracking-wider text-white/50">Coverage</div>
              <div className="mt-1 font-mono text-white/85">
                {typeof coverageDays !== "undefined" && coverageDays !== null ? `${coverageDays} days` : "—"}
              </div>
            </div>
          </div>
          ...
          {typeof isGated !== "undefined" && isGated ? (
            <div className="mt-3 text-xs text-white/70">
              Interpretive outputs (regimes + notables) are suppressed while one or more quality gates are active. Charts and the
              latest snapshot remain visible.
            </div>
          ) : null}
        </div>
      </div>
      ...
      {/* Charts (filtered by chain) */}
            ...
            <details className="mt-6 rounded-2xl border border-white/10 bg-white/5 p-5" onToggle={(e) => setChartsOpen((e.currentTarget as HTMLDetailsElement).open)}>
        <summary className="cursor-pointer select-none text-sm font-semibold text-white/90">
          Charts (click to expand)
          <span className="ml-2 text-xs font-normal text-white/60">MA7 / MA30 context • descriptive</span>
        </summary>
