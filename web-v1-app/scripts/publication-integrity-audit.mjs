...
// D-116 Checkout response cache boundary final audit
{
  const d116CheckoutRouteFile = path.join(root, "src", "app", "api", "v1", "checkout", "route.ts");
  const d116CheckoutSource = fs.existsSync(d116CheckoutRouteFile)
    ? fs.readFileSync(d116CheckoutRouteFile, "utf8").replace(/^\uFEFF/u, "")
    : "";

  function d116LooseFunctionRegion(sourceText, functionName, fallbackLength = 7000) {
    const starts = [
  ...
  const d116JsonErrorRegion = d116LooseFunctionRegion(d116CheckoutSource, "jsonError", 2600);
  const d116GetRegion = d116LooseFunctionRegion(d116CheckoutSource, "GET", 2200);
  const d116PostRegion = d116LooseFunctionRegion(d116CheckoutSource, "POST", 3200);
  const d116HandleRegion = d116LooseFunctionRegion(d116CheckoutSource, "handleCheckout", 12000);

  const d116NoStoreHeaderLiteralExists =
    d116CheckoutSource.includes("Cache-Control") &&
    d116CheckoutSource.includes("no-store");

  const d116JsonErrorIsNoStore =
    d116JsonErrorRegion.includes("NextResponse.json") &&
    d116JsonErrorRegion.includes("Cache-Control") &&
    d116JsonErrorRegion.includes("no-store");

  const d116GetMethodNotAllowedIsNoStore =
    !d116CheckoutSource.includes("export async function GET") ||
    (
      d116GetRegion.includes("method_not_allowed") &&
      d116GetRegion.includes("405") &&
      d116GetRegion.includes("Cache-Control") &&
      d116GetRegion.includes("no-store")
    );

  const d116RedirectsAreNoStore =
    !d116HandleRegion.includes("NextResponse.redirect") ||
    (
      d116HandleRegion.includes("Cache-Control") &&
      d116HandleRegion.includes("no-store")
    );

  const d116StripeSessionRedirectIsNoStore =
    !d116HandleRegion.includes("session.url") ||
    (
      d116HandleRegion.includes("NextResponse.redirect") &&
      d116HandleRegion.includes("session.url") &&
      d116HandleRegion.includes("Cache-Control") &&
  ...
  const d116SigninRedirectIsNoStore =
    !(
      d116HandleRegion.includes("sign-in") ||
      d116HandleRegion.includes("redirect_url")
    ) ||
    (
      d116HandleRegion.includes("NextResponse.redirect") &&
      d116HandleRegion.includes("Cache-Control") &&
  ...
  const d116NoRawNextJsonInRuntimePaths =
    !/NextResponse\.json\s*\(/u.test(
      (d116HandleRegion + "\n" + d116PostRegion).replace(d116JsonErrorRegion, "")
    );

  const d116GetDirectJsonIsBoundedIfPresent =
    !d116GetRegion.includes("NextResponse.json") ||
    (
      d116GetRegion.includes("method_not_allowed") &&
      d116GetRegion.includes("Cache-Control") &&
      d116GetRegion.includes("no-store")
  ...
  const d116NoPublicCacheDirectives =
    !/\bCache-Control["']?\s*:\s*["'][^"']*(?:public|s-maxage|max-age|immutable|stale-while-revalidate)/iu.test(
      d116CheckoutSource
    );

  const d116ValidationAndFailureResponsesUseJsonError =
    d116JsonErrorIsNoStore &&
    ["invalid_plan", "checkout_price_not_configured", "checkout_redirect_origin_not_configured", "checkout_not_configured", "stripe_error"].some((code) =>
      d116CheckoutSource.includes(code)
    );

  const d116PostIngressUsesNoStoreHelper =
    d116JsonErrorIsNoStore &&
    d116PostRegion.includes("validateSameOriginRequest") &&
    d116PostRegion.includes("enforcePreAuthRateLimit");

  const d116NoSensitiveCacheableResponse =
    !/\b(?:session\.url|stripeCustomerId|stripeSubscriptionId|STRIPE_SECRET_KEY|STRIPE_PRICE_|DATABASE_URL)\b[\s\S]{0,260}\bCache-Control["']?\s*:\s*["'][^"']*(?:public|s-maxage|max-age|immutable)/iu.test(
      d116CheckoutSource
    );

  const d116Checks = [
    ["CHECKOUT_NO_STORE_HEADER_LITERAL_MISSING", d116NoStoreHeaderLiteralExists, "checkout route must define no-store Cache-Control response headers."],
    ["CHECKOUT_JSON_ERROR_NO_STORE_MISSING", d116JsonErrorIsNoStore, "checkout jsonError helper must set no-store."],
    ["CHECKOUT_GET_METHOD_NOT_ALLOWED_NO_STORE_MISSING", d116GetMethodNotAllowedIsNoStore, "checkout GET method_not_allowed response must be no-store."],
    ["CHECKOUT_SIGNIN_REDIRECT_NO_STORE_MISSING", d116SigninRedirectIsNoStore, "checkout sign-in redirect must be no-store."],
    ["CHECKOUT_STRIPE_SESSION_REDIRECT_NO_STORE_MISSING", d116StripeSessionRedirectIsNoStore, "Stripe Checkout session redirect must be no-store."],
    ["CHECKOUT_REDIRECT_NO_STORE_COVERAGE_MISSING", d116RedirectsAreNoStore, "checkout redirects must set no-store."],
    ["CHECKOUT_JSON_ERROR_HELPER_BYPASS_RISK", d116NoRawNextJsonInRuntimePaths && d116GetDirectJsonIsBoundedIfPresent, "checkout JSON error responses must go through no-store jsonError helper, except bounded GET 405."],
    ["CHECKOUT_PUBLIC_CACHE_DIRECTIVE_RISK", d116NoPublicCacheDirectives, "checkout route must not emit public/s-maxage/max-age/immutable cache directives."],
    ["CHECKOUT_VALIDATION_FAILURE_NO_STORE_MISSING", d116ValidationAndFailureResponsesUseJsonError, "checkout validation/failure responses must be no-store."],
    ["CHECKOUT_POST_INGRESS_NO_STORE_BOUNDARY_MISSING", d116PostIngressUsesNoStoreHelper, "checkout ingress failures must use no-store response helper."],
    ["CHECKOUT_SENSITIVE_RESPONSE_CACHE_RISK", d116NoSensitiveCacheableResponse, "checkout sensitive response paths must not be cacheable."]
  ];

  for (const [code, ok, detail] of d116Checks) {
    if (!ok) {
      result.findings.push({
        severity: "fail",
        auditItem: "D-116",
