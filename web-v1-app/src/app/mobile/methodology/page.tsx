      ...
      <main className="flex-1 space-y-4 px-4 py-4 pb-24">
        <div className="rounded-2xl border border-cyan-500/15 bg-cyan-500/[0.04] p-4">
          <p className="text-[13px] leading-[1.7] text-slate-200">
            This is the short version. It tells you what the model does, what the labels mean,
            and how to read the output without forcing the full desktop documentation onto a phone screen.
          </p>
        </div>

        <div className="space-y-3">
          {cards.map((card) => (
            <section key={card.title} className="rounded-2xl border border-white/8 bg-white/[0.03] p-4">
              <div className="text-[14px] font-bold text-white">{card.title}</div>
              <p className="mt-2 text-[12px] leading-[1.7] text-slate-300">{card.body}</p>
            </section>
          ))}
        </div>
        ...
        <div className="rounded-2xl border border-white/8 bg-white/[0.03] p-4">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            ...
            <Link href="/methodology?view=desktop" className="rounded-full border border-white/10 bg-white/5 px-3 py-1.5 text-[11px] font-semibold text-slate-200">
              Open full desktop methodology
            </Link>
          </div>
        </div>
      </main>
