...
export default async function MobileChainPage({
  ...
  // Simplified mobile surface: always show the latest 30d published history.
  const windowDays: 30 = 30;
      ...
      <main className="flex-1 space-y-4 px-4 py-4 pb-24">
        <div className="rounded-2xl border border-cyan-500/15 bg-cyan-500/5 px-4 py-3">
          <p className="text-[11px] leading-[1.65] text-slate-200">
            This mobile chain view is a simplified version of the full desktop page.
            It is optimized for quick reading on a phone and currently shows the latest
            published state plus the recent 30-day historical view.
          </p>
          <a
            href={`/chains/${chainId}?view=desktop`}
            className="mt-2 inline-block text-[11px] font-semibold text-cyan-300"
          >
            Open full desktop chain page →
          </a>
        </div>
        ...
        <div className="rounded-2xl border border-white/8 bg-white/[0.03] p-4">
          ...
          <div className="mt-3 text-[11px] text-slate-500">
            Need deeper historical context? Open the desktop chain page for the full
            experience.
          </div>
        </div>
