      ...
      <main className="flex-1 space-y-4 px-4 py-4 pb-24">
        <div className="rounded-2xl border border-cyan-500/15 bg-cyan-500/[0.04] p-4">
          <p className="text-[13px] leading-[1.7] text-slate-200">
            This is the mobile proof layer: recent labels, recent confidence, and enough history
            to show whether a condition was persistent or just a one-day spike.
          </p>
        </div>

        {summaries.map((summary) => (
          <section key={summary.chain} className="rounded-2xl border border-white/8 bg-white/[0.03] p-4">
            <div className="flex items-start justify-between gap-3">
              ...
              <Link href={`/mobile/chain/${summary.chain}`} className="text-[11px] font-semibold text-cyan-300">
                Open chain →
              </Link>
            </div>
            ...
            <div className="mt-4 space-y-2">
              {summary.rows.slice(0, 10).map((row) => {
                ...
                return (
                  <div key={row.date} className="rounded-xl border border-white/6 bg-black/15 px-3 py-2.5">
                    ...
                    <div className="mt-1 text-[11px] text-slate-400">
                      Confidence {typeof row.confidence === "number" ? row.confidence.toFixed(3) : "—"}
                    </div>
                    {row.oneLiner ? <div className="mt-1 text-[11px] leading-[1.6] text-slate-500">{row.oneLiner}</div> : null}
                  </div>
                );
              })}
            </div>
