...
function CodeBlock({ children, label }: { children: string; label?: string }) {
  return (
    <div>
      {label && <div className="mb-1.5 text-[10px] font-bold uppercase tracking-wider text-[var(--urd-text-muted)]">{label}</div>}
      <pre className="overflow-x-auto rounded-2xl border bg-[var(--urd-panel-strong)] p-5 text-xs leading-6 text-[var(--urd-text-body)]">
        <code>{children}</code>
      </pre>
    </div>
  );
...
function Section({ eyebrow, title, subtitle, children, id }: {
  ...
  return (
    <section id={id} className="scroll-mt-24 rounded-3xl border p-6 shadow-sm">
      ...
      {subtitle && <div className="mt-2 max-w-4xl text-sm leading-7 text-[var(--urd-text-body)]">{subtitle}</div>}
...
function Callout({ children, color = "cyan" }: { children: ReactNode; color?: "cyan" | "amber" | "emerald" }) {
  ...
  return <div className={`rounded-2xl border p-5 text-sm leading-7 text-[var(--urd-text-body)] ${s[color]}`}>{children}</div>;
}
    ...
    <main className="mx-auto max-w-7xl px-6 py-10">
      ...
      <header className="mb-10">
        <div className="rounded-3xl border bg-[radial-gradient(circle_at_top_left,rgba(34,211,238,0.10),transparent_40%)] p-8 shadow-sm">
          ...
          {/* Table of contents */}
          <div className="mt-8 rounded-2xl border border-[var(--urd-border-soft)] bg-[var(--urd-raised)] p-5">
            <div className="text-xs font-bold uppercase tracking-wider text-[var(--urd-text-muted)] mb-3">Contents</div>
      ...
      <ShortFullContent
        ...
        <Section id="what-is-json" eyebrow="Chapter 1" title="What JSON is">
          <p className="text-sm leading-7 text-[var(--urd-text-body)]">
    ...
    { "metric": "median_tx_fee_native", "z_robust": -2.02 }
          ...
          <div className="mt-5 grid gap-4 lg:grid-cols-3">
            {[
              ...
              <div key={item.title} className="rounded-2xl border bg-[var(--urd-raised)] p-4">
                <div className="text-sm font-bold text-[var(--urd-text-strong)] font-mono">{item.title}</div>
                <p className="mt-2 text-xs leading-5 text-[var(--urd-text-muted)]">{item.desc}</p>
              </div>
            ))}
          </div>
          <Callout color="cyan">
            <span className="font-semibold text-[var(--urd-text-strong)]">Key point:</span>{" "}
        ...
        <Section id="what-is-api" eyebrow="Chapter 2" title="What an API is">
          ...
          <div className="mt-5 rounded-2xl border border-[var(--urd-border-soft)] bg-[var(--urd-raised)] p-5">
            <h3 className="text-sm font-bold text-[var(--urd-text-strong)] mb-4">The difference — with and without an API</h3>
            <div className="grid gap-4 lg:grid-cols-2">
              <div className="rounded-xl border border-white/6 p-4">
                <div className="text-xs font-bold uppercase tracking-wider text-[var(--urd-text-muted)] mb-2">Without API</div>
                ...
                <div className="mt-2 text-xs text-[var(--urd-text-muted)]">Time per day: 10–30 minutes. Error-prone. Cannot be automated.</div>
              </div>
              <div className="rounded-xl border border-cyan-500/15 bg-cyan-500/5 p-4">
                ...
                <div className="mt-2 text-xs text-[var(--urd-text-muted)]">Time per day: seconds. Fully automated. No manual steps.</div>
              </div>
            </div>
          </div>
          <div className="mt-5">
            <h3 className="text-sm font-bold text-[var(--urd-text-strong)] mb-3">What an API key is</h3>
            <p className="text-sm leading-7 text-[var(--urd-text-body)]">
            ...
            <Callout color="amber">
              <span className="font-semibold text-[var(--urd-text-strong)]">Important:</span>{" "}
        ...
        <Section id="what-you-get" eyebrow="Chapter 3" title="What you actually receive as a subscriber">
          ...
          <div className="mt-5 overflow-x-auto">
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="border-b border-[var(--urd-border-soft)]">
                  <th className="text-left py-3 pr-6 text-xs font-bold uppercase tracking-wider text-[var(--urd-text-muted)]">Genre</th>
                  <th className="text-left py-3 pr-6 text-xs font-bold uppercase tracking-wider text-[var(--urd-text-muted)]">What it contains</th>
                  <th className="text-left py-3 text-xs font-bold uppercase tracking-wider text-[var(--urd-text-muted)]">Best for</th>
                </tr>
              </thead>
              <tbody className="text-[var(--urd-text-body)]">
                <tr className="border-b border-white/5">
                  ...
                  <td className="py-3 text-[var(--urd-text-muted)]">Your own calculations, raw data analysis</td>
                </tr>
                <tr className="border-b border-white/5">
                  ...
                  <td className="py-3 text-[var(--urd-text-muted)]">Dashboards, alerts, research conditioning</td>
                </tr>
                <tr>
                  ...
                  <td className="py-3 text-[var(--urd-text-muted)]">Trend charts, smoothed analysis</td>
                </tr>
              </tbody>
            </table>
          </div>
          ...
          <div className="mt-6">
            <h3 className="text-sm font-bold text-[var(--urd-text-strong)] mb-3">Files available per plan</h3>
            <div className="grid gap-4 lg:grid-cols-2">
              <div className="rounded-2xl border border-cyan-500/20 bg-cyan-500/5 p-5">
                <div className="text-xs font-bold uppercase tracking-wider text-blue-700 mb-3">Basic — $29/mo — 1 chain</div>
                <p className="text-xs text-[var(--urd-text-muted)] mb-3">If your entitled chain is bitcoin, you can fetch:</p>
...
/api/v1/files/derived/bitcoin/90d/latest.json`}</CodeBlock>
                <p className="mt-3 text-xs text-[var(--urd-text-muted)]">That is 12 files total per day — 3 genres × 4 windows.</p>
              </div>
              <div className="rounded-2xl border border-purple-500/20 bg-purple-500/5 p-5">
                <div className="text-xs font-bold uppercase tracking-wider text-purple-300 mb-3">Pro — $79/mo — all 4 chains</div>
                <p className="text-xs text-[var(--urd-text-muted)] mb-3">Same structure, but for all four chains and 6 windows (adds 180d and 365d):</p>
...
...and so on for gold and derived`}</CodeBlock>
                <p className="mt-3 text-xs text-[var(--urd-text-muted)]">That is 72 files total — 3 genres × 4 chains × 6 windows.</p>
              </div>
            </div>
          </div>
        </Section>
...
/api/v1/files/{genre}/{chain}/latest.json`}</CodeBlock>
          ...
          <div className="mt-6 grid gap-4">
            <QA question="What does 'latest.json' (without a window) mean?">
              ...
              <span className="block mt-2 font-semibold text-[var(--urd-text-strong)]">Fetch this every day to always have the newest data.</span>
            </QA>
            ...
            <QA question="What does '90d/latest.json' mean?">
              ...
              <div className="mt-3 rounded-xl border border-white/6 bg-[var(--urd-raised)] p-4">
  ...
  "days": [
    ...
    ...90 records total
              ...
              <span className="block mt-2 font-semibold text-[var(--urd-text-strong)]">
            ...
            <QA question="Which file should I fetch every day?">
              <span className="font-semibold text-[var(--urd-text-strong)]">For daily monitoring:</span> fetch <InlineCode>latest.json</InlineCode> (without a window).
              One request per chain, one file, always fresh.
              <br /><br />
              <span className="font-semibold text-[var(--urd-text-strong)]">For building a historical dataset:</span> fetch <InlineCode>90d/latest.json</InlineCode>
              once when you sign up to get your history, then fetch <InlineCode>latest.json</InlineCode>
              daily to add one new day at a time.
              <br /><br />
              <span className="font-semibold text-[var(--urd-text-strong)]">For backtesting or analysis over a long period:</span>{" "}
            ...
            <QA question="What are the available windows?">
              <div className="mt-1 overflow-x-auto">
                <table className="w-full text-xs border-collapse">
                  <thead>
                    <tr className="border-b border-[var(--urd-border-soft)]">
                      <th className="text-left py-2 pr-4 text-[var(--urd-text-muted)]">Window token</th>
                      <th className="text-left py-2 pr-4 text-[var(--urd-text-muted)]">What it contains</th>
                      <th className="text-left py-2 text-[var(--urd-text-muted)]">Available on</th>
                    </tr>
                  </thead>
                  <tbody className="text-[var(--urd-text-body)]">
                    {[
                      ...
                      <tr key={token} className="border-b border-white/5">
                        ...
                        <td className="py-2 text-[var(--urd-text-muted)]">{plan}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
        ...
        <Section id="daily-or-manual" eyebrow="Chapter 5" title="Do I get the data automatically, or do I have to fetch it manually?">
          ...
          <div className="mt-5 grid gap-4 lg:grid-cols-2">
            <div className="rounded-2xl border bg-[var(--urd-raised)] p-5">
              <h3 className="text-sm font-bold text-[var(--urd-text-strong)] mb-3">What Urd Atlas does automatically</h3>
              <BulletList items={[
            ...
            <div className="rounded-2xl border bg-[var(--urd-raised)] p-5">
              <h3 className="text-sm font-bold text-[var(--urd-text-strong)] mb-3">What you do</h3>
              <BulletList items={[
          ...
          <div className="mt-5 rounded-2xl border border-[var(--urd-border-soft)] bg-[var(--urd-raised)] p-5">
            <h3 className="text-sm font-bold text-[var(--urd-text-strong)] mb-4">Two ways subscribers work</h3>
            <div className="grid gap-4 lg:grid-cols-2">
              <div>
                <div className="text-xs font-bold uppercase tracking-wider text-[var(--urd-text-muted)] mb-2">Manual (start here)</div>
                <p className="text-sm leading-6 text-[var(--urd-text-body)]">
              ...
              <div>
                <div className="text-xs font-bold uppercase tracking-wider text-[var(--urd-text-muted)] mb-2">Automated (next step)</div>
                <p className="text-sm leading-6 text-[var(--urd-text-body)]">
  ...
  -o bitcoin_meta_latest.json`}</CodeBlock>
          ...
          <Callout color="emerald">
            <span className="font-semibold text-[var(--urd-text-strong)]">Total time for steps 1–6:</span>{" "}
        ...
        <Section id="first-request" eyebrow="Chapter 7" title="How to make your first API request">
          ...
          <div className="mt-5 grid gap-5">
            <div className="rounded-2xl border bg-[var(--urd-raised)] p-5">
              <h3 className="text-base font-semibold text-[var(--urd-text-strong)] mb-1">Option A: curl (simplest — works on Windows, Mac, Linux)</h3>
              <p className="text-xs text-[var(--urd-text-muted)] mb-3">curl is usually pre-installed. Open any terminal and run:</p>
              <CodeBlock>{`curl -H "X-API-Key: YOUR_KEY_HERE" https://urdatlas.com/api/v1/files/meta/bitcoin/latest.json`}</CodeBlock>
              <p className="mt-3 text-xs text-[var(--urd-text-muted)]">To save to a file instead of printing:</p>
            ...
            <div className="rounded-2xl border bg-[var(--urd-raised)] p-5">
              <h3 className="text-base font-semibold text-[var(--urd-text-strong)] mb-1">Option B: Python (recommended for analysis)</h3>
              <p className="text-xs text-[var(--urd-text-muted)] mb-3">Install requests once: <InlineCode>pip install requests</InlineCode></p>
...
with open("meta_bitcoin_latest.json", "w") as f:
    json.dump(data, f, indent=2)`}</CodeBlock>
            ...
            <div className="rounded-2xl border bg-[var(--urd-raised)] p-5">
              <h3 className="text-base font-semibold text-[var(--urd-text-strong)] mb-1">Option C: PowerShell (Windows)</h3>
...
$response | ConvertTo-Json -Depth 10 | Out-File -FilePath "meta_bitcoin_latest.json"`}</CodeBlock>
        ...
        <Section id="tools" eyebrow="Chapter 8" title="What software you need on your computer">
          ...
          <div className="mt-5 grid gap-4 lg:grid-cols-3">
            <div className="rounded-2xl border bg-[var(--urd-raised)] p-5">
              <h3 className="text-sm font-bold text-[var(--urd-text-strong)]">Minimum (already have)</h3>
              <BulletList items={[
              ...
              <p className="mt-3 text-xs text-[var(--urd-text-muted)]">Good enough for: manual daily fetching and reading the output.</p>
            </div>
            <div className="rounded-2xl border border-cyan-500/15 bg-cyan-500/5 p-5">
              <h3 className="text-sm font-bold text-[var(--urd-text-strong)]">Recommended for analysis</h3>
              <BulletList items={[
              ...
              <p className="mt-3 text-xs text-[var(--urd-text-muted)]">Good for: building your own analysis, charts, and automation.</p>
            </div>
            <div className="rounded-2xl border bg-[var(--urd-raised)] p-5">
              <h3 className="text-sm font-bold text-[var(--urd-text-strong)]">For non-coders</h3>
              <BulletList items={[
              ...
              <p className="mt-3 text-xs text-[var(--urd-text-muted)]">Good for: spreadsheet users who want structured data without writing code.</p>
            </div>
          </div>
        </Section>
        ...
        <Section id="analysis" eyebrow="Chapter 9" title="How to analyse the data — practical examples">
          ...
          <div className="mt-5 grid gap-5">
            <div className="rounded-2xl border bg-[var(--urd-raised)] p-5">
              <h3 className="text-base font-semibold text-[var(--urd-text-strong)] mb-3">Read the regime label and confidence from a file</h3>
...
print(f"{date}: {label} (confidence: {confidence:.2f})")`}</CodeBlock>
            ...
            <div className="rounded-2xl border bg-[var(--urd-raised)] p-5">
              <h3 className="text-base font-semibold text-[var(--urd-text-strong)] mb-3">Load a 90-day history file into a table (pandas)</h3>
              <CodeBlock>{`import json
import pandas as pd
...
print(df["label"].value_counts())  # how many days each label appeared`}</CodeBlock>
            ...
            <div className="rounded-2xl border bg-[var(--urd-raised)] p-5">
              <h3 className="text-base font-semibold text-[var(--urd-text-strong)] mb-3">Compare all four chains (Pro) — build a daily summary table</h3>
...
print(df.to_string(index=False))`}</CodeBlock>
            ...
            <div className="rounded-2xl border bg-[var(--urd-raised)] p-5">
              <h3 className="text-base font-semibold text-[var(--urd-text-strong)] mb-3">Load into Excel — no code required</h3>
              <BulletList items={[
              ...
              <p className="mt-3 text-xs text-[var(--urd-text-muted)]">
        ...
        <Section id="automation" eyebrow="Chapter 10" title="How to automate daily fetching">
          <Callout>
            <span className="font-semibold text-[var(--urd-text-strong)]">Do not automate until the manual version works.</span>{" "}
            First: fetch one file manually. Then: fetch it in a script. Then: schedule that script.
          </Callout>

          <div className="mt-5">
            <h3 className="text-base font-semibold text-[var(--urd-text-strong)] mb-3">The recommended beginner automation pattern</h3>
            <p className="text-sm leading-7 text-[var(--urd-text-body)] mb-4">
...
python3 fetch_urdatlas.py`}</CodeBlock>
          ...
          <div className="mt-5 grid gap-4 lg:grid-cols-2">
            <div className="rounded-2xl border bg-[var(--urd-raised)] p-5">
              <h3 className="text-sm font-bold text-[var(--urd-text-strong)] mb-3">Schedule on Windows — Task Scheduler</h3>
              <BulletList items={[
            ...
            <div className="rounded-2xl border bg-[var(--urd-raised)] p-5">
              <h3 className="text-sm font-bold text-[var(--urd-text-strong)] mb-3">Schedule on Mac / Linux — cron</h3>
              <p className="text-xs text-[var(--urd-text-muted)] mb-3">Open Terminal and type <InlineCode>crontab -e</InlineCode>, then add:</p>
