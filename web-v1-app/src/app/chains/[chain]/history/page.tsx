...
function isSupportedChain(v: string): v is ChainId { return CHAINS.includes(v as ChainId); }
function expectedDelayDays(chain: ChainId) { return chain === "arbitrum" || chain === "base" ? 7 : 1; }
...
async function readPublishedJson<T>(path: string): Promise<T | null> {
  ...
  try { return JSON.parse(decodeBody(result.body)) as T; } catch { return null; }
}
...
function fmtDate(v?: string) { return v?.trim() ? v : "—"; }
function fmtConf(v?: number) { return typeof v === "number" ? v.toFixed(3) : "—"; }
...
export default async function ChainHistoryPage({
  ...
  const latestConfidence = typeof latestRow?.confidence?.confidence_score === "number"
    ? latestRow.confidence.confidence_score : null;
  const latestLag = typeof latestRow?.confidence?.lag_days_vs_utc_today === "number"
    ? latestRow.confidence.lag_days_vs_utc_today : null;
  const latestRegime = latestRow?.status?.label ?? null;

  const goodCount     = rows.filter(r => (r.confidence?.confidence_score ?? 0) >= 0.7).length;
  const cautionCount  = rows.filter(r => { const s = r.confidence?.confidence_score ?? 0; return s >= 0.4 && s < 0.7; }).length;
      ...
      <header className="hero border-b border-[var(--line)]">
        <div className="page-shell">
          <div className="eyebrow mb-4">
            <Link href="/chains" className="text-link">Chains</Link>
            <span className="mx-2 text-[var(--ink3)]">/</span>
            <Link href={`/chains/${chainId}`} className="text-link">{chainLabel}</Link>
            <span className="mx-2 text-[var(--ink3)]">/</span>
            <span className="text-[var(--ink3)]">History</span>
          </div>
          <h1 className="ua-h1">{chainLabel} <em>history</em></h1>
          ...
          <div className="page-shell">
            <div className="fact-row" style={{ gridTemplateColumns: "repeat(6,1fr)" }}>
              ...
              <div className="fact-item">
                <strong>Band</strong>
                <span>
                  <span className={`regime-token ${bandClass(confidenceBand(latestConfidence ?? undefined))}`}>
              ...
              <div className="fact-item">
                ...
                <span>{expectedDelayDays(chainId)}d</span>
              </div>
            </div>
          </div>
        </section>
      ...
      <div className="page-shell py-16">
        ...
        {rows.length === 0 ? (
          <div className="border-y border-[var(--gold-line)] py-6">
            <div className="eyebrow mb-2 text-[var(--gold)]">Bundle unavailable</div>
            <p className="text-sm text-[var(--ink2)] max-w-xl">
              The canonical history bundle for this chain could not be read from published
              storage. This page does not fall back to alternate files.
            </p>
          </div>
        ) : (
          <>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {rows.map((row, index) => {
                    ...
                    return (
                      <tr key={`${row.date ?? "row"}-${index}`} className="border-b border-[var(--line)] hover:bg-[var(--surface3)] transition-colors">
                        <td className="py-4 pr-6 font-mono text-[12px] text-[var(--ink)]">{fmtDate(row.date)}</td>
                        <td className="py-4 pr-6 font-mono text-[12px] text-[var(--ink2)]">{fmtDate(row.updated_through ?? row.regime?.asof_date)}</td>
                        <td className="py-4 pr-6">
                          <span className={`regime-token ${regimeClass(row.status?.label ?? null)}`}>
                            {row.status?.label ?? "—"}
                          </span>
                        </td>
                        <td className="py-4 pr-6 font-mono text-[12px] text-[var(--ink)]">{fmtConf(row.confidence?.confidence_score)}</td>
                        <td className="py-4 pr-6">
                          <span className={`regime-token ${bandClass(band)}`}>{band}</span>
                        </td>
                        <td className="py-4 pr-6 font-mono text-[12px] text-[var(--ink2)]">
                          {typeof row.confidence?.lag_days_vs_utc_today === "number" ? `${row.confidence.lag_days_vs_utc_today}d` : "—"}
                        </td>
                        <td className="py-4 text-[var(--ink2)] text-[13px] max-w-xs">{row.status?.one_liner?.trim() || "—"}</td>
                      </tr>
                    );
                  })}
                </tbody>
        ...
        <div className="mt-12 border-t border-[var(--line)] pt-8">
          ...
          <div className="data-row" style={{ gridTemplateColumns: "160px 1fr" }}>
            <span className="font-mono text-[11px] text-[var(--ink3)] uppercase tracking-[.1em]">Source file</span>
            <span className="font-mono text-[12px] text-[var(--ink2)]">{canonicalPath}</span>
          </div>
          <div className="data-row" style={{ gridTemplateColumns: "160px 1fr" }}>
            <span className="font-mono text-[11px] text-[var(--ink3)] uppercase tracking-[.1em]">Expected delay</span>
            <span className="font-mono text-[12px] text-[var(--ink2)]">{expectedDelayDays(chainId)}d</span>
          </div>
        </div>
        ...
        <div className="mt-12 border-t border-[var(--line)] pt-8 grid gap-0 max-w-2xl">
          ...
          <div className="data-row"><p>This history view is descriptive only. It does not recompute regime labels.</p></div>
          <div className="data-row"><p>It does not imply forecast quality or backtested performance.</p></div>
          <div className="data-row"><p>Read together with <Link href="/methodology" className="text-link">methodology</Link>, <Link href="/glossary" className="text-link">glossary</Link>, and the current <Link href={`/chains/${chainId}`} className="text-link">chain page</Link>.</p></div>
          <div className="data-row"><p><Link href="/track-record" className="text-link">Cross-chain track record →</Link></p></div>
        </div>
        ...
        <div className="mt-12 border-y border-[var(--line)] py-6 flex flex-wrap items-center justify-between gap-6">
          <div>
            ...
            <p className="text-sm text-[var(--ink2)] max-w-lg">
              ...
              A subscription gives you API access to the complete Meta JSON.
            </p>
          </div>
