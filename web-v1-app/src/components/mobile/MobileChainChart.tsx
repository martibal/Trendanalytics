...
function RegimeYAxisTick(props: unknown) {
  const p = props as { x?: number; y?: number; payload?: { value?: number } };
  const y = typeof p.y === "number" ? p.y : 0;
  const value = Number(p.payload?.value ?? 0);
  const label = REGIME_VALUE_LABELS[value] ?? "UNK";
  const color =
    REGIME_COLORS[label === "UNK" ? "UNKNOWN/DEGRADED" : label] ?? "#6B7280";
  return (
        ...
        <div className="mb-2 flex items-center justify-between gap-3">
          ...
          {selected ? (
            <div className="text-right text-[11px]">
              ...
              <div className="font-black" style={{ color: selected.color }}>
                {selected.label ?? "—"}
              </div>
            </div>
          ) : (
            <div className="text-[10px] text-slate-600">Tap to inspect</div>
          )}
        </div>
      ...
      <div className="rounded-2xl border border-white/8 bg-black/15 p-3">
        <div className="mb-2 flex items-center justify-between gap-3">
          ...
          {selected ? (
            ...
            <div className="text-[10px] text-slate-600">0.40 = publish gate</div>
          )}
        </div>
