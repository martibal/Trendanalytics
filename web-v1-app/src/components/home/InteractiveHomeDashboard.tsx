...
export default function InteractiveHomeDashboard({ snapshots, lastRun, heroSnapshot }: Props) {
  ...
  const selectedPointIndex = selectedHistoryIndex == null ? Math.max(0, selectedHistory.length - 1) : Math.min(selectedHistoryIndex, Math.max(0, selectedHistory.length - 1));
  const selectedPoint = selectedHistory[selectedPointIndex] ?? null;
  const graphPoints = useMemo(() => {
    if (selectedHistory.length < 2) return "";
    return selectedHistory.map((row, index) => `${((index / (selectedHistory.length - 1)) * 960).toFixed(1)},${regimeY(row.label)}`).join(" ");
  }, [selectedHistory]);
  const cursorX = selectedHistory.length > 1 ? (selectedPointIndex / (selectedHistory.length - 1)) * 100 : 100;

  useEffect(() => { setSelectedHistoryIndex(null); }, [selectedChainId]);
  ...
  function handleGraphClick(event: MouseEvent<SVGSVGElement>) {
    if (selectedHistory.length < 2 || !chartRef.current) return;
    const rect = chartRef.current.getBoundingClientRect();
    const ratio = Math.max(0, Math.min(1, (event.clientX - rect.left) / rect.width));
    setSelectedHistoryIndex(Math.round(ratio * (selectedHistory.length - 1)));
  }
    ...
    <main className="ua5">
      <section className="ua5-state-strip" aria-label="Latest network state">
        ...
        <div className="ua5-state-scroll">
          {snapshots.map((chain) => (
            <div key={chain.id} className={`ua5-state-item ua5-status-${chain.regime.toLowerCase().replace("/", "-")}`}>
              <button type="button" className="ua5-state-select" aria-pressed={selectedChainId === chain.id} onClick={() => setSelectedChainId(chain.id)}><strong>{chain.ticker}</strong><span>{chain.regime}</span><small>{chain.confidence}</small></button>
      ...
      <section className="ua5-hero">
        <div className="ua5-shell ua5-hero-grid">
          ...
          <div className="ua5-chart-wrap">
            <div className="ua5-chain-tabs" role="group" aria-label="Select chain">
              {snapshots.map((chain) => <button key={chain.id} type="button" className={chain.id === selectedChainId ? "is-active" : ""} onClick={() => setSelectedChainId(chain.id)}>{chain.ticker}</button>)}
            </div>
            <svg ref={chartRef} className="ua5-chart" viewBox="0 0 960 470" preserveAspectRatio="none" onClick={handleGraphClick} aria-label={`${selectedChain.name} regime history. Select a point to inspect that day.`}>
              ...
              {selectedHistory.length > 1 ? <><line x1={`${cursorX}%`} y1="0" x2={`${cursorX}%`} y2="470" className="ua5-cursor" /><circle cx={`${cursorX}%`} cy={regimeY(displayRegime)} r="6" className="ua5-cursor-dot" /></> : null}
            </svg>
      ...
      <section id="why" className="ua5-light-section"><div className="ua5-shell">
        ...
        <div className={`ua5-data-scene ${showContext ? "is-expanded" : ""}`}><h3>Your data</h3><p>Click once. The Urd Atlas columns expand into the same table.</p><div className="ua5-data-table"><div className="ua5-data-row ua5-data-head"><span>Date</span><span>Model error</span><span className="ua5-added">Regime</span><span className="ua5-added">Confidence</span></div><div className="ua5-data-row"><span>14 Aug</span><span>2.1%</span><span className="ua5-added">STABLE</span><span className="ua5-added">88%</span></div><div className="ua5-data-row"><span>15 Aug</span><span>2.0%</span><span className="ua5-added">STABLE</span><span className="ua5-added">91%</span></div><div className="ua5-data-row"><span>16 Aug</span><span>4.3%</span><span className="ua5-added">CHEAP</span><span className="ua5-added">92%</span></div></div><button className="ua5-context-button" type="button" onClick={() => setShowContext((value) => !value)}>{showContext ? "Remove Urd Atlas columns" : "Add Urd Atlas columns"}</button><p className="ua5-data-conclusion">Now you know the anomaly coincided with a different network regime. That does not prove causality — it tells you where to investigate.</p></div>
      </div></section>
      ...
      <section id="product" className="ua5-product"><div className="ua5-shell">
        ...
        <div className="ua5-product-grid"><div className="ua5-observation"><small>PUBLISHED NETWORK STATE · {selectedChain.name.toUpperCase()} · {selectedChain.asOf}</small><h3>{selectedChain.regime} <InfoButton name={selectedChain.regime} onOpen={setExplainer} /></h3><p>{selectedChain.oneLiner}</p>{([['demand',selectedChain.demand],['friction',selectedChain.friction],['capacity',selectedChain.capacity]] as const).map(([name,value]) => <div className="ua5-metric" key={name}><label>{name} <InfoButton name={name} onOpen={setExplainer} /></label><span><i style={{ width: `${Math.max(0, Math.min(100, value ?? 0))}%` }} /></span><b>{value == null ? "—" : value.toFixed(1)}</b></div>)}<div className="ua5-metric"><label>confidence <InfoButton name="confidence" onOpen={setExplainer} /></label><span><i style={{ width: `${Math.round((selectedChain.confidenceValue ?? 0) * 100)}%` }} /></span><b>{selectedChain.confidence}</b></div></div>
          <div className="ua5-files"><div className="ua5-file-tabs">{(Object.keys(artifactCopy) as Artifact[]).map((artifact) => <span className="ua5-file-tab-wrap" key={artifact}><button type="button" className={artifact === selectedArtifact ? "is-active" : ""} onClick={() => setSelectedArtifact(artifact)}>{artifactCopy[artifact].title}</button><InfoButton name={`artifact-${artifact}`} onOpen={setExplainer} /></span>)}<button className="ua5-full-json" type="button" onClick={() => setJsonOpen(true)}>View full JSON</button></div><div className="ua5-file-meta"><span>{selectedChain.id} / latest / {selectedArtifact.toLowerCase()}.json</span><span>method {selectedChain.methodologyVersion}</span></div><pre className="ua5-code"><code>{compactJson(jsonPayload)}</code></pre><p className="ua5-file-purpose">{artifactCopy[selectedArtifact].short}</p></div>
        </div>
      </div></section>
      ...
      {jsonOpen ? <div className="ua5-modal-backdrop" onClick={() => setJsonOpen(false)}><div className="ua5-modal" role="dialog" aria-modal="true" aria-labelledby="ua5-json-title" onClick={(event) => event.stopPropagation()}><header><div><small>COMPLETE PUBLISHED JSON</small><h2 id="ua5-json-title">{artifactCopy[selectedArtifact].title} · {selectedChain.name}</h2></div><button type="button" onClick={() => setJsonOpen(false)}>Close</button></header><div className="ua5-modal-tabs">{(Object.keys(artifactCopy) as Artifact[]).map((artifact) => <button key={artifact} type="button" className={artifact === selectedArtifact ? "is-active" : ""} onClick={() => setSelectedArtifact(artifact)}>{artifactCopy[artifact].title}</button>)}</div><pre><code>{JSON.stringify(selectedChain.artifacts[selectedArtifact] ?? null, null, 2)}</code></pre></div></div> : null}

      {activeExplanation || explainer?.startsWith("artifact-") ? <aside className="ua5-explainer" role="dialog" aria-live="polite"><button type="button" onClick={() => setExplainer(null)}>×</button>{explainer?.startsWith("artifact-") ? <><strong>{artifactCopy[explainer.replace("artifact-", "") as Artifact].title}</strong><p>{artifactCopy[explainer.replace("artifact-", "") as Artifact].explanation}</p></> : activeExplanation ? <><strong>{activeExplanation.title}</strong><p>{activeExplanation.body}</p></> : null}</aside> : null}
    </main>
  );
}
