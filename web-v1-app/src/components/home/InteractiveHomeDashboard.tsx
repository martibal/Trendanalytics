...
export default function InteractiveHomeDashboard({ snapshots, lastRun }: Props) {
  ...
  return (
    <main className="relative overflow-hidden bg-background text-foreground">
      ...
      <section className="border-y border-white/10 bg-white/[0.018]"><div className="mx-auto grid max-w-7xl gap-4 px-6 py-8 lg:grid-cols-4 lg:px-8">{[["Evidence", "Validation diagnostics before purchase", "/validation", "emerald"], ["Prototype", "Free CSV calendar and starter notebook", "/analyst-kit", "cyan"], ["Integrate", "Public checks then authenticated delivery", "/api-docs", "amber"], ["Boundary", "Descriptive context, not automated instructions", "/methodology", "violet"]].map(([title, body, href, color]) => (<Link key={title} href={href} className="group rounded-3xl border border-white/10 bg-black/20 p-5 transition hover:-translate-y-1 hover:border-white/25 hover:bg-white/[0.045]"><p className={`font-mono text-[10px] uppercase tracking-[0.18em] ${color === "emerald" ? "text-emerald-200" : color === "cyan" ? "text-cyan-200" : color === "amber" ? "text-amber-200" : "text-violet-200"}`}>{title}</p><p className="mt-3 text-sm leading-6 text-zinc-400">{body}</p></Link>))}</div></section>
      ...
      <section className="mx-auto grid max-w-7xl gap-8 px-6 py-16 lg:grid-cols-[1.1fr_0.9fr] lg:px-8"><div className="rounded-[2rem] border border-white/10 bg-white/[0.035] p-5 shadow-2xl shadow-black/30 backdrop-blur-2xl"><div className="mb-4 flex items-center justify-between border-b border-white/10 pb-4"><p className="font-mono text-[10px] uppercase tracking-[0.18em] text-cyan-200">Trust model</p><div className="flex gap-1.5"><span className="h-2 w-2 rounded-full bg-red-400/70" /><span className="h-2 w-2 rounded-full bg-amber-400/70" /><span className="h-2 w-2 rounded-full bg-emerald-400/70" /></div></div><div className="rounded-3xl border border-white/10 bg-black/30 p-6 text-center"><p className="font-mono text-[clamp(16px,2.5vw,30px)] text-white">Confidence = √(Data Quality × Label Confidence)</p></div><div className="mt-5 grid gap-3 sm:grid-cols-3">{[["Data Quality", "Coverage, freshness and missingness context."], ["Label Confidence", "How strongly the row supports the published regime."], ["Point-in-time", "Observation date and available-at context stay separate."]].map(([title, body]) => (<div key={title} className="rounded-2xl border border-white/10 bg-white/[0.025] p-4 transition hover:-translate-y-1 hover:border-white/20"><p className="font-mono text-[10px] uppercase tracking-[0.16em] text-cyan-200">{title}</p><p className="mt-2 text-xs leading-5 text-zinc-500">{body}</p></div>))}</div></div><div className="rounded-[2rem] border border-emerald-200/20 bg-[linear-gradient(135deg,rgba(16,185,129,.10),rgba(56,189,248,.055))] p-7 shadow-[0_0_70px_rgba(16,185,129,.09)] backdrop-blur-2xl"><p className="font-mono text-xs uppercase tracking-[0.18em] text-emerald-200">Free versus paid</p><h2 className="mt-4 text-3xl font-semibold tracking-[-0.03em] text-white sm:text-4xl">Free is for proof. Paid is for delivery.</h2><p className="mt-4 text-sm leading-7 text-zinc-400">Inspect Explorer, Validation, Methodology and the public Analyst Kit first. Upgrade only when authenticated recurring files are useful for your workflow.</p><div className="mt-6 flex flex-wrap gap-3"><Link href="/plans" className="inline-flex items-center justify-center rounded-full border border-cyan-300/40 bg-cyan-300/15 px-5 py-3 font-mono text-xs font-medium uppercase tracking-[0.12em] text-white transition hover:-translate-y-0.5 hover:border-cyan-200/70">View plans</Link><Link href="/dashboard" className="inline-flex items-center justify-center rounded-full border border-white/10 bg-white/[0.035] px-5 py-3 font-mono text-xs font-medium uppercase tracking-[0.12em] text-zinc-300 transition hover:-translate-y-0.5 hover:border-white/25 hover:text-white">Open dashboard</Link></div></div></section>
