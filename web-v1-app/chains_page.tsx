...
function buildChartDataDense(params: {
  ...
  // Forward-fill MA values across gaps in the published data.
  // The pipeline may leave null entries for days it could not compute moving
  // averages (e.g. pipeline gaps, late-arriving data). Carrying the last known
  // value forward produces a continuous line that accurately represents the
  // trend, rather than a misleading flat bridge (connectNulls) or blank gap.
  let lastMa7: number | null = null;
  let lastMa30: number | null = null;
  return raw.map((pt) => {
    if (pt.ma7 != null) lastMa7 = pt.ma7;
    if (pt.ma30 != null) lastMa30 = pt.ma30;
    return {
      ...pt,
      ma7: pt.ma7 ?? lastMa7,
      ma30: pt.ma30 ?? lastMa30,
    };
  });
}

function normalizeLevel(q?: string): "Basic" | "Advanced" {
...
export default async function ChainPage({
  ...
  const levelBasicHref = `/chains/${chainId}?level=basic&window=${requestedWindow}`;
        ...
        <div className="flex flex-col gap-3">
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <Link href={levelAdvancedHref} prefetch={false}>
            ...
            {charts.map((c) => (
              <MetricLineChart
                ...
                subtitle={`MA: ${derivedPath} · Raw: ${goldPath} · Window: ${utcMsToIsoDay(
                  bounds.minMs
                )} → ${utcMsToIsoDay(bounds.maxMs)} (${effectiveWindowDays} calendar days) · MA gaps forward-filled`}
              ...
              <div key={`${d.metric ?? "driver"}-${i}`} className="rounded-xl border p-6">
                ...
                <div className="mt-4">
                  <DriverExplanation
                    ...
                    trend={d.trend}
                    z={d.z_robust}
                    percentile90d={d.pct_90d}
                    momentum={d.momentum_7d_vs_30d}
                  />
                </div>
              </div>
            ))}
